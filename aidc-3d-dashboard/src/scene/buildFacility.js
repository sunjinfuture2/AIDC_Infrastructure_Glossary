...
function buildSite() {
  ...
  /* 지상면(GL) 가장자리 소산 — 지상 평지 면 자체가 경계로 갈수록 흰색으로
     흩어지는 그라운드 베일. 경계 밖은 알파 1로 이어져 굴토 단면·하부 슬래브
     돌출부도 가려진다. 전체·1층 뷰에서만 표시 (siteDetail 규칙 공유) */
  const veil = groundVeil(g, -14, -10, GL + 0.12, 152, 126, 60, 13)
